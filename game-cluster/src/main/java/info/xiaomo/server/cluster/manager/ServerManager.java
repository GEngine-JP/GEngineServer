...
public class ServerManager {
     ...
     * 服务器列表
     */
    final Map<Integer, Map<Integer, ServerInfo>> servers = new ConcurrentHashMap<>();
    ...
    public ServerInfo registerServer(GameServerInfo serverInfo, IoSession session) {
        Map<Integer, ServerInfo> map = servers.get(serverInfo.getType());
        if (map == null) {
            map = new ConcurrentHashMap<>();
            servers.put(serverInfo.getType(), map);
        }
     ...
     * @return
     */
    public Map<Integer, ServerInfo> getServers(Integer serverType) {
     ...
     * @return
     */
    public ServerInfo getServer(Integer serverType, int serverId) {
    ...
    public Map<Integer, Map<Integer, ServerInfo>> getServers() {
        return servers;
    }
